...
const app = getApp()
const { addHelpActivity, canUseCardList, uploadImg, updateHelpActivity } = api
...
Page({
    ...
    saveData(){
        ...
        if( showKey == 'card_id'){
            ...
            showlist[4].value = selectInfo.name
            showlist[4].selectId = selectInfo.id
            showlist[5].value = selectInfo.use_type == 1 ? '约课生效' : '立即生效'
            showlist[5].selectId = selectInfo.use_type + ''
        }
        ...
        if(showKey == 'card_use_type'){
            showlist[5].value = checkedName
            showlist[5].selectId = checkedId.toString()
        }
    ...
    sure(){
        ...
        let submitData = {}
        let reqUrl = actId ? updateHelpActivity : addHelpActivity
    ...
    getImg() {
        ...
        wx.chooseImage({
            ...
            success: function (res) {
                ...
                // that.cropper.pushImg(imgInfo.content.url);
                ...
                wx.uploadFile({
                    ...
                    success: function(res){
                        ...
                        if(res.statusCode == 200) {
                            ...
                            that.data.showlist[13].value = that.data.alyunFilePath
                            that.setData({
    ...
    //     wx.uploadFile({
    //       url: baseUrl + uploadImg,
    //       filePath: that.data.filePath[that.data.uploadFileIdx],
    //       name: 'image',
    //       formData:{
    //         image:that.data.filePath[that.data.uploadFileIdx]
    //       },
    //       success: function(res){
    //         let imgInfo = JSON.parse(res.data)
    //         if(res.statusCode == 200) {
    //           that.data.upyunFilePath.push(imgInfo.content.url)
    //           that.data.showlist[3].value = that.data.upyunFilePath[0]
    //           that.setData({
    //             showlist:that.data.showlist
    //           })
    //           wx.hideLoading()
    ...
    //         wx.uploadFile({
    //             url: baseUrl + uploadImg,
    //             filePath: that.data.imgFileList[that.data.alloadFileIdx],
    //             name: 'image',
    //             formData:{
    //             image:that.data.imgFileList[that.data.alloadFileIdx]
    //             },
    //             success: function(res){
    //             let imgInfo = JSON.parse(res.data)
    //             if(res.statusCode == 200) {
    //                 that.data.alyunFilePath.push(imgInfo.content.url)
    //                 that.setData({
    //                     alloadFileIdx: parseInt(that.data.alloadFileIdx) + 1
    //                 })
    //                 if(that.data.alloadFileIdx <= that.data.imgFileList.length - 1) {
    //                     that.uploadFiles()
    //                     return reject()
    //                 }else{
    //                     that.data.showlist[13].value = that.data.alyunFilePath
