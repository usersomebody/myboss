 ...
 * 
 全局公共方法
 ps:可能存在因项目的业务逻辑而进行的相应封装的操作方法
*/
//获取星期
function weekDayOfWeek(day){
    let x = '一'
    switch (day) { 
      case 0:
        x="日"; 
...
function filterVlaue(validRoute = ['1','3','7'], asyncRoutes = []){
    ...
    const accessedRouters = routeMap.filter(v => {
        if (this.getMenu(validRoute, v.link)) {
          if (v.children && v.children.length > 0) {
            v.children = v.children.filter(child => {
              if (this.getMenu(validRoute, child.link)) {
                return child
              }
              return false;
            });
...
function setStoreId(id){
    getApp().globalData.isShareIn = true
